...
const NAV: NavItem[] = [
  { label: "Explore", to: "/explore", icon: Compass, items: [{ label: "Gallery", to: "/gallery" }, { label: "Blogs & Magazine", to: "/blogs" }] },
  { label: "Learning", to: "/learning", icon: GraduationCap, items: [{ label: "Online Courses", to: "/learn" }, { label: "Schools", to: "/schools" }] },
  { label: "Shop", to: "/tools", icon: ShoppingBag, items: [{ label: "Tools", to: "/tools", badge: "SALE" }, { label: "Arts Sales", to: "/arts", badge: "NEW" }] },
  { label: "Network", to: "/network", icon: NetworkIcon, items: [{ label: "Jobs", to: "/jobs" }, { label: "Hiring", to: "/hire" }] },
];

function Badge({ kind }: { kind: "SALE" | "NEW" }) {
  return <span className="badge ml-2" style={{ background: kind === "SALE" ? "var(--color-badge-sale)" : "var(--color-badge-new)", color: "#fff" }}>{kind}</span>;
}

function Logo() {
  return (
    <Link to="/" className="select-none text-[1.15rem] font-semibold tracking-tight leading-none" aria-label="ArtSpace — home">
...
function SubItem({ s, onClick }: { s: SubLink; onClick?: () => void }) {
  return (
    <Link to={s.to} hash={s.hash} onClick={onClick} className="flex items-center justify-between px-3 py-2 text-sm hover:bg-[var(--color-surface-2)] transition-colors">
      <span>{s.label}</span>
...
function NavDropdown({ item }: { item: NavItem }) {
  ...
  const onEnter = () => { if (timer.current) window.clearTimeout(timer.current); setOpen(true); };
...
function SearchTrigger({ onOpen, className }: { onOpen: () => void; className?: string }) {
  return (
    <button onClick={onOpen} className={cn("relative w-full max-w-[340px] flex items-center h-9 rounded-lg border px-3 bg-[var(--color-surface)] text-left hover:border-[var(--color-accent)] transition-colors", className)} style={{ borderColor: "var(--color-border)" }} aria-label="Open search">
      ...
      <aside className={cn("fixed top-0 left-0 z-[9999] flex h-screen w-[82%] max-w-[320px] flex-col border-r border-[var(--color-border)] bg-[var(--color-background)] text-[var(--color-foreground)]", closing ? "animate-drawer-out" : "animate-drawer")}>
        ...
        <div className="flex items-center justify-between gap-2 px-4 py-4 border-b border-[var(--color-border)]">
          ...
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <button onClick={onClose} aria-label="Close menu" className="h-10 w-10 grid place-items-center rounded-lg border border-[var(--color-border)] hover:bg-[var(--color-surface)] transition-colors">
        ...
        <div className="px-4 py-4">
          <SearchTrigger onOpen={() => { onClose(); onOpenSearch(); }} className="w-full max-w-full" />
        </div>
        ...
        <nav className="flex flex-1 flex-col overflow-y-auto px-4 pb-6">
          <div className="space-y-3">
            {NAV.map((item, index) => {
              ...
              return (
                <div key={item.label} className="pt-3" style={index === 0 ? undefined : { borderTop: "1px solid color-mix(in oklab, var(--color-border) 15%, transparent)" }}>
                  <button
                    type="button"
                    onClick={() => toggleSection(item.label)}
                    className={cn(
                      "flex w-full items-center justify-between gap-3 border-l-4 pl-3 pr-2 py-1 text-left text-[1.1rem] font-medium leading-tight text-[var(--color-foreground)] transition-colors duration-200",
                      expanded ? "border-l-[var(--color-accent)]" : "border-l-transparent hover:border-l-[var(--color-accent)] hover:text-[var(--color-accent)]",
                    )}
                  >
                  ...
                  {expanded && (
                    <div className="space-y-1 pb-3 pt-2" style={{ paddingLeft: "1.75rem" }}>
                      {item.items.map((s) => (
                        <Link
                          key={s.label}
                          to={s.to}
                          hash={s.hash}
                          onClick={onClose}
                          className="flex items-center justify-between py-2 text-[0.9rem] font-medium text-[var(--color-muted-foreground)] transition-colors hover:text-[var(--color-foreground)]"
                        >
                          <span>{s.label}</span>
                          {s.badge ? (
        ...
        {/* Bottom buttons */}
        <div className="mt-auto px-4 pb-6 pt-4 border-t border-[var(--color-border)]">
          {user ? (
            <button onClick={() => { onClose(); onSignOut(); }} className="btn btn-ghost w-full">Sign out</button>
          ) : (
            <div className="space-y-3">
...
function UserMenu() {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen((v) => !v)} className="h-9 w-9 rounded-full grid place-items-center text-sm font-medium text-white" style={{ background: "linear-gradient(135deg, var(--color-accent), color-mix(in oklab, var(--color-accent) 50%, #000))" }} aria-label="Account menu">
        {initials}
      </button>
      {open && (
        <div className="dropdown-panel animate-dropdown absolute right-0 top-full mt-2 min-w-[220px] py-2 z-50">
          ...
          <button onClick={async () => { await signOut(); setOpen(false); toast.success("Signed out"); }} className="w-full text-left flex items-center gap-2 px-3 py-2 text-sm hover:bg-[var(--color-surface-2)]">
...
export function Header() {
  ...
  return (
    <header className={cn("sticky top-0 z-50 w-full backdrop-blur-md transition-colors", scrolled ? "bg-[color-mix(in_oklab,var(--color-background)_85%,transparent)]" : "bg-[var(--color-background)]")} style={{ borderBottom: "1px solid var(--color-border)" }}>
      <div className="mx-auto max-w-[1400px] px-4 lg:px-6 h-14 flex items-center gap-4">

        <button onClick={() => setDrawer(true)} aria-label="Open menu" className="lg:hidden h-9 w-9 grid place-items-center rounded-lg hover:bg-[var(--color-surface)] -ml-1">
        ...
        <div className="flex items-center gap-2 lg:hidden">
          ...
          {user ? <UserMenu /> : <button onClick={() => openAuth("signin")} className="btn btn-ghost h-9 px-3 text-xs">Sign in</button>}
          <ThemeToggle />
        </div>
      </div>

      <MobileDrawer open={drawer} onClose={() => setDrawer(false)} onOpenSearch={() => setSearchOpen(true)} onSignIn={() => openAuth("signin")} onSignUp={() => openAuth("signup")} user={user} onSignOut={async () => { await signOut(); toast.success("Signed out"); }} />
